...
internal static class TableDdl
{
    ...
    /// <summary>
    /// CREATE-context column definition from a rich FieldDto (no ADD prefix, no AFTER
    /// positioning). Per dialect: type+length, unsigned/zerofill (mysql), nullable,
    /// the auto-increment keyword (mysql AUTO_INCREMENT / mssql IDENTITY / pg
    /// GENERATED BY DEFAULT AS IDENTITY / sqlite AUTOINCREMENT), DEFAULT (omitted for
    /// identity — pg/mssql reject identity+default), comment+collation (mysql), pg array.
    /// </summary>
    internal static string RenderCreateColumnDef(string client, FieldDto f)
    {
        var name = QuoteIdent(client, f.Name);
        var typeUpper = f.Type.ToUpperInvariant();
        var lengthSpec = BuildLengthSpec(f);
        var def = f.AutoIncrement == true ? string.Empty : RenderDefault(f);
        return client switch
        {
            "mssql" => $"{name} {typeUpper}{lengthSpec}"
                + (f.AutoIncrement == true ? " IDENTITY(1,1)" : string.Empty)
                + (f.Nullable == false ? " NOT NULL" : " NULL")
                + def,
            "mysql" or "maria" => $"{name} {typeUpper}{lengthSpec}"
            ...
            "pg" => $"{name} {typeUpper}{lengthSpec}"
                + (f.IsArray == true ? "[]" : string.Empty)
                + (f.AutoIncrement == true ? " GENERATED BY DEFAULT AS IDENTITY" : string.Empty)
                + (f.Nullable == false ? " NOT NULL" : string.Empty)
                + def,
            // quirk: sqlite `INT AUTOINCREMENT` (+ a separate PRIMARY KEY clause) is
            // invalid — sqlite requires `INTEGER PRIMARY KEY AUTOINCREMENT` inline.
            // Pre-existing behavior, locked by TableDdlCreateTests; sqlite auto-increment
            // create needs a dedicated path (follow-up).
            _ => $"{name} {typeUpper}{lengthSpec}"
                + (f.AutoIncrement == true ? " AUTOINCREMENT" : string.Empty)
                + (f.Nullable == false ? " NOT NULL" : string.Empty)
                + def,
        };
    }
    ...
    /// <summary>
    /// `CREATE TABLE {qualified} (col defs[, PRIMARY KEY (...)])` from rich fields plus
    /// an explicit primary-key column list (the renderer carries PK as a PRIMARY entry
    /// in the indexes array). Non-primary indexes / FKs / checks / comment are applied
    /// as post-steps by the caller.
    /// </summary>
    internal static string RenderCreateTableFromFields(string client, string qualifiedTable,
        IReadOnlyList<FieldDto> fields, IReadOnlyList<string> primaryKeyCols)
    {
        var cols = string.Join(", ", fields.Select(f => RenderCreateColumnDef(client, f)));
        if (primaryKeyCols.Count > 0)
            cols += $", PRIMARY KEY ({string.Join(", ", primaryKeyCols.Select(c => QuoteIdent(client, c)))})";
